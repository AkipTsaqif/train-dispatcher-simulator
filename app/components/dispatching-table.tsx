/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect, react-hooks/purity, react-hooks/immutability */
"use client";
...
const articulatedBodyPath = (
	...
	const normals: [number, number][] = spine.map((_, i) => {
		...
		if (i === spine.length - 1)
			return unit(spine.length - 2, spine.length - 1);
...
const spineOf = (
	...
	const segLen = Math.hypot(
		st.segTo[0] - st.segFrom[0],
		st.segTo[1] - st.segFrom[1],
	);
	...
	const push = (pt: [number, number]) => {
		...
		if (!last || Math.hypot(pt[0] - last[0], pt[1] - last[1]) > 1e-9)
			route.push(pt);
	};
	...
	for (let i = 0; i + 1 < route.length; i++) {
		...
		const t = Math.max(
			...
			Math.min(
				1,
				((center[0] - ax) * dx + (center[1] - ay) * dy) / (len * len),
			),
		);
		const px = ax + dx * t;
		const py = ay + dy * t;
	...
	for (let i = 0; i + 1 < route.length; i++) {
		...
		if (i + 1 < route.length - 1)
			vertices.push({ s: acc, pt: route[i + 1] });
	}
	for (let i = vertices.length - 1; i >= 0; i--) {
		const v = vertices[i];
		if (v.s < noseS && v.s > tailS)
			spine.push([...v.pt] as [number, number]);
	}
	spine.push(at(tailS));
	return spine;
};
...
const withoutStraightJoints = (
	...
	for (let i = 1; i < spine.length - 1; i++) {
		...
		if (Math.abs((ax * by - ay * bx) / (al * bl)) > 1e-6)
			kept.push(spine[i]);
	}
...
const walkRoute = (
	...
	const segLvl = (a: string | null, b: string | null) =>
		a && b
			? (map.segmentLevels[`${a}|${b}`] ??
				map.segmentLevels[`${b}|${a}`] ??
				0)
			: 0;
	const pts: LeveledPoint[] = [
		[sig.x, sig.y, segLvl(sig.edge[0], sig.edge[1])],
	];
	...
	const nextSignalOn = (
		...
		return cands.reduce((a, b) =>
			dir === "right" ? (a.x < b.x ? a : b) : a.x > b.x ? a : b,
		);
	};
	...
	while (cur && nodes[cur]) {
		...
		if (node.sw !== undefined) {
			...
			const branchExit = node.exits.find(
				(exit) => exit.viaSwitchPort === "reversed",
			);
			const branchAhead =
				branchExit !== undefined &&
				bearingDot(incBearing, branchExit.bearing) > 0;
			...
			const cameFromBranch =
				branchExit !== undefined && incoming === branchExit.neighbor;
			if (cameFromBranch) {
				if (!reversed) {
					push(node.x, node.y);
				...
				if (
					branchExit!.farSw === undefined ||
					switches[branchExit!.farSw] === "reversed"
				) {
				...
				return done(
					`berakhir di P${branchExit!.farSw} — belum diatur`,
					undefined,
					true,
				);
			} else if (reversed) {
		...
		const reversed =
			node.sw !== undefined && switches[node.sw] === "reversed";
		let bestExit: GNodeExit | undefined;
		let bestDot = -Infinity;
...
const calculateSignalAspect = (
	...
	visited.add(id);
	const signal = dependencies.signals.find(
...
const STATION_OPTIONS = [
	{
		...
		badge: "24 Jam",
		description: "8 Jalur Peron · Lintas Percabangan",
		dispatch: JATINEGARA_DISPATCH,
	},
	{
		id: "bekasi",
		code: "BKS",
		name: "Lintas Bekasi Timur – Tambun – Cibitung",
		badge: "24 Jam",
		description: "3 Stasiun · 4 Jalur Peron Utama",
...
export default function DispatchingTable({
	dispatch: initialDispatch = BEKASI_TAMBUN_CIBITUNG_DISPATCH,
}: { dispatch?: DispatchRuntime } = {}) {
	const [currentDispatch, setCurrentDispatch] =
		useState<DispatchRuntime>(initialDispatch);
	const [selectedStationId, setSelectedStationId] = useState<string>(() =>
		initialDispatch.map.stations.nameplates.some((s) => s.code === "JNG")
			? "jng"
			: "jng",
	);
	const {
	...
	const FLANK_CLEARANCE =
		DISPATCH_MAP.interlocking?.flankClearance ??
		DISPATCH_MAP.grid.cellSize / 3;
	...
	const CLOSED_SPANS: ClosedSpan[] = (DISPATCH_MAP.outOfService ?? []).map(
		(span) => {
			const main = DISPATCH_MAP.lines.mains.find(
	...
	// does its job.
	const ERASER_SCALE = Math.max(TRACK_STROKE + 0.25, 3.5 * CELL_RATIO) / 3.5;
	...
	const {
		...
		signals: { items: SIGNALS, initialState: INITIAL_SIGNALS },
	...
	const TRACK_CLIPS =
		DISPATCH_SCENARIO.spawn?.atTrackEdge === true
			? JOURNEYS.map(({ plan }) => {
					...
					return xs.length
						? {
								lo: Math.min(...xs) - SHIFT,
								hi: Math.max(...xs) - SHIFT,
							}
						: null;
				})
			: null;
	...
	const SECTION_PATHS = DISPATCH_MAP.sectionPaths as Record<
		string,
		LeveledPoint[]
	>;
	...
	useEffect(() => {
		const saved =
			typeof localStorage !== "undefined"
				? localStorage.getItem("ppka-dark")
				: null;
		const pref = saved
			? saved === "1"
	...
	type Notice = {
		...
		kind?: "susul" | "countdown" | "approach"; // susul warning (amber), approach (amber), departure countdown (blue), or a signal hold (red)
	};
	...
	const meetsReadRef = useRef<
		(partnerIdx: number, stopIdx: number) => number | null
	>(() => null);
	...
	const distToPoly = (px: number, py: number, pts: LeveledPoint[]) => {
		...
		for (let i = 0; i + 1 < pts.length; i++) {
			...
			best = Math.min(
				best,
				Math.hypot(px - (x1 + dx * t), py - (y1 + dy * t)),
			);
		}
		return best;
	};
	...
	const unpassedOf = (
		...
		const owner =
			ownerIdx !== undefined
				? trainStatesRef.current[ownerIdx]
				: undefined;
		...
		if (owner && owner.spawned) {
			fronts = [
				{
					...
					front:
						owner.dir === "left"
							? owner.x + RESERVATION_REAR_OFFSET
							: owner.x - RESERVATION_REAR_OFFSET,
				},
			];
		} else {
			...
			fronts = trainStatesRef.current
				...
				.map((m) => ({
					lineY: m.y,
					front: m.dir === "left" ? m.x + CELL : m.x - CELL,
				}));
		}
	...
	switchesRef.current = switches;
	const aspectOfRef = useRef<(id: string, selfIdx?: number) => Aspect>(
		() => "red",
	);
	...
	const initializeSim = (
		sec: number,
		targetDispatch: DispatchRuntime = currentDispatch,
	) => {
		simRef.current = sec;
		const {
			map: mapDef,
			journeys: journeysList,
			meetsByTrain: meetsMap,
			movement: moveDef,
		} = targetDispatch;

		if (targetDispatch !== currentDispatch) {
			setCurrentDispatch(targetDispatch);
			setSwitches(targetDispatch.map.switches.initialState);
			setSignalOn(targetDispatch.map.signals.initialState);
			setReservations({});
			setNotices([]);
			setClickLog([]);
			setSelectedTrain(null);
		...
		const mkCtx = (): MoveCtx => ({
			...moveDef,
			switches:
				targetDispatch === currentDispatch
					? switches
					: targetDispatch.map.switches.initialState,
			aspectOf: () => "red",
			ignoreSignals: true,
		});
		const place = (ctx: MoveCtx) => {
			journeysList.forEach((j, ti) => {
				...
				const spawnAt = targetDispatch.scenario.dwell.relativeAnchors
					? plan.boundaryArr
					: plan.originArr;
				if (sec < spawnAt) {
					st.spawned = false;
				...
				spawnSimRef.current[ti] = targetDispatch.scenario.dwell
					...
					? spawnAt
					: sec;
				lastTickRef.current[ti] = targetDispatch.scenario.dwell
					.relativeAnchors
					? spawnAt
					: sec;
				if (targetDispatch.scenario.dwell.relativeAnchors) {
					...
					if (elapsed > 0 && !st.done)
						advanceTrain(st, elapsed, placeCtx, plan.legs);
		...
		// Initialize trainStates for targetDispatch before pass 1
		trainStatesRef.current = journeysList.map((j, ti) => {
			const n = initTrain(
				j.plan,
				moveDef.nodes,
				j.plan.legs[0]?.speed ?? 0,
			...
			n.idx = ti;
			n.actualArr = j.train.stops.map(() => null);
			return n;
		});
		...
		journeysList.forEach((j, ti) => {
			const st = trainStatesRef.current[ti];
			if (meetsMap.has(ti) && st.spawned && !st.done) {
				const origin = j.train.stops[0].trackmark;
				const originX = mapDef.stations.platformCenterX[origin];
				st.susulWarned =
					st.dir === "right" ? st.x > originX : st.x < originX;
			}
		});
		setPaused(false);
	...
	useEffect(() => {
		...
		const tick = () => {
			...
			const ctx: MoveCtx = {
				...
				meetsHold,
				now: simRef.current,
				driverReactionSeconds:
					DISPATCH_SCENARIO.driver?.reactionSeconds ?? 0,
				stopFrameAtDwellArrival: RELATIVE_CLOCK,
			};
			// partner crossings come from the live states each tick
			meetsReadRef.current = (pi, si) =>
				trainStatesRef.current[pi]?.actualArr[si] ?? null;
			...
			const live = trainStatesRef.current.filter(
				(m) => m.spawned && !m.done,
			);
			const atRest = (m: TrainState): boolean => {
				...
				return base > 0 || meets > 0
					? m.time < Math.max(base, meets)
					: false; // dwelling
			};
			...
			const leaderOf = (a: TrainState, b: TrainState): TrainState => {
				if (a.x !== b.x)
					return (a.dir === "right" ? a.x > b.x : a.x < b.x) ? a : b;
			...
			if (newConflict && !conflictReportedRef.current) {
				...
				logClick(
					`⚠ conflict — two trains overlap on the track${conflictPair ? ` (${conflictPair})` : ""}`,
				);
			...
			const sections = JOURNEYS.map((j, ti) => {
				...
				for (const pid of st.passedSignals) {
					...
					if (!s.block && !s.ai) {
						...
						if (reservationsRef.current[s.id])
							reservedByRef.current[s.id] = ti;
						setSignalOn((o) =>
							o[s.id] ? { ...o, [s.id]: false } : o,
						);
					}
					for (const [k, v] of Object.entries(
						...
						if (
							v.nextSignalId === s.id &&
							reservedByRef.current[k] === undefined
						) {
				...
				if (st.stopped && st.stopReason === "signal") {
					...
					if (st.notificationId !== null) {
						const dur = Math.max(
							0,
							Math.round(simRef.current - st.holdSince),
						);
						setNotices((ns) =>
							ns.map((x) =>
								x.id === st.notificationId
									? {
											...x,
											resolved: true,
				...
				if (!st.susulWarned) {
					...
					if (departed) {
						st.susulWarned = true;
						if (NOTIFICATION_POLICY.notifyOnApproach) {
							const msg =
								NOTIFICATION_POLICY.approachMessage ??
								`sudah mendekati ${stationName(origin)}!`;
							setNotices((ns) =>
								[
									{
										id: nextNoticeIdRef.current++,
										kind: "approach" as const,
										trainNo: `KA ${j.train.train_no}`,
										message: msg,
							...
							const passers = (
								MEETS_BY_TRAIN.get(ti)?.get(
									NOTIFICATION_POLICY.susulMeetStation ?? "",
								) ?? []
				...
				if (st.actualArr[1] !== null) {
					setNotices((ns) => {
						const hasOpen = ns.some(
							(x) =>
								x.kind === "approach" &&
								x.trainNo === `KA ${j.train.train_no}` &&
								!x.resolved,
						);
						if (!hasOpen) return ns;
						return ns.map((x) =>
							x.kind === "approach" &&
							x.trainNo === `KA ${j.train.train_no}` &&
							!x.resolved
				...
				if (
					...
					const leg =
						plan.legs[Math.min(st.leg, plan.legs.length - 1)];
					const atCountdownStation =
						leg.station === countdownPolicy.station &&
						st.time < (leg.departAt ?? 0);
					...
					if (
						...
						setNotices((ns) => {
							const existing = ns.find(
								(x) =>
									x.kind === "countdown" && x.trainNo === key,
							);
							if (existing) {
				...
				for (const [id, res] of Object.entries(
					...
					let user =
						userIdx !== undefined
							? trainStatesRef.current[userIdx]
							: undefined;
					...
					if (!hadUser) {
						...
						trainStatesRef.current.forEach((m, i) => {
							if (!m.spawned || m.done || m.dir !== ownerDir)
								return;
					...
					const rear = user
						? ownerDir === "left"
							? user.x + RESERVATION_REAR_OFFSET
							: user.x - RESERVATION_REAR_OFFSET
						: 0;
					...
					const leftRoute =
						!!user &&
						!user.done &&
						distToPoly(user.x, user.y, res.pts) > CELL * 1.5;
				...
				const leadFactor =
					easeX === null || frontCompensation <= 0
						...
						: Math.min(
								1,
								Math.abs(st.x - easeX) / frontCompensation,
							);
				const horizontal = st.segFrom[1] === st.segTo[1];
				const continuousMarker =
					IS_SCHEMATIC && PRESENTATION.continuousTrains;
				...
				if (continuousMarker) {
					...
					const visualCenter =
						...
						(st.dir === "right"
							...
							: -engineToVisualFront);
					const raw =
						(visualCenter + SHIFT - markerXOrigin) / markerPitch;
					...
					const len = Math.hypot(dx, dy) || 1;
					const dominantAxis =
						Math.max(Math.abs(dx), Math.abs(dy)) / len || 1;
					...
					const progress =
						Math.hypot(st.x - fx, st.y - fy) +
						frontCompensation * leadFactor;
					const stepped = Math.min(
						len,
						Math.round(progress / diagonalPitch) * diagonalPitch,
					);
					const t = stepped / len;
					rx = fx + dx * t;
					ry = fy + dy * t;
				...
				const rawSpine =
					ARTICULATED_TRAINS && !continuousMarker
						? spineOf(
								...
								[rx, ry],
								VISUAL_HALF_LEN,
								pathAhead(st, ctx, VISUAL_HALF_LEN),
							)
						: null;
				const bendySpine = rawSpine
					? withoutStraightJoints(rawSpine)
					: null;
				...
				const stroke =
					selectedIdxRef.current === ti ? "#f59e0b" : c.stroke;
				if (rect) {
					if (rect.getAttribute("fill") !== c.fill)
						rect.setAttribute("fill", c.fill);
					if (rect.getAttribute("stroke") !== stroke)
						rect.setAttribute("stroke", stroke);
					// the straight box and the bendy body are alternatives, never both
					rect.setAttribute(
						"visibility",
						bendy ? "hidden" : "visible",
					);
				}
				const bendyEl = trainBendyRefs.current[ti];
				if (bendyEl) {
					bendyEl.setAttribute(
						"visibility",
						bendy ? "visible" : "hidden",
					);
					if (bendy) {
						...
						bendyEl.setAttribute(
							"d",
							articulatedBodyPath(local, TRAIN_HALF_HEIGHT),
						);
						if (bendyEl.getAttribute("fill") !== c.fill)
							bendyEl.setAttribute("fill", c.fill);
						if (bendyEl.getAttribute("stroke") !== stroke)
							bendyEl.setAttribute("stroke", stroke);
					}
				}
				...
				if (arrow) {
					...
					if (bendy && bendySpine && bendySpine.length >= 2) {
						...
						if (segLen > 1e-9) {
							leadAng =
								(Math.atan2(nly - bly, nlx - blx) * 180) /
								Math.PI;
							...
							const tipDist =
								ARROW_TIP_FRAC * TRAIN_HALF_LEN * axial;
				...
				if (exclam) {
					exclam.setAttribute(
						"visibility",
						conflictIdx.has(st.idx) ? "visible" : "hidden",
					);
					exclam.setAttribute(
						"transform",
						bendy
	...
	const coupledWith = (id: number): number[] =>
		COUPLED.find((g) => g.includes(id)) ?? [id];

	/** A coupled pair is locked when either end is locked by a route. */
	const isLocked = (id: number): boolean =>
		coupledWith(id).some((gid) => lockedBy(gid) !== undefined);

	const toggleSwitch = (id: number) => {
		const group = coupledWith(id);
		const owner = group
			.map((gid) => lockedBy(gid))
			.find((o) => o !== undefined);
		if (owner) {
			const name =
				group.length > 1 ? `P${group.join("+P")}` : `P${group[0]}`;
			setConflictNote(
				`${name} terkunci oleh reservasi ${codeOf(owner)} — ubah ${codeOf(owner)} menjadi merah dulu.`,
			);
		...
		logClick(
			`${group.length > 1 ? `P${group.join("+P")}` : `P${group[0]}`} → ${newState === "reversed" ? "BELOK" : "LURUS"}`,
		);
	};
	...
	const toggleSignal = (id: string, exitId?: string) => {
		...
		if (!found) {
			...
			window.setTimeout(() => setConflictNote(null), 3000);
			logClick(
				`${codeOf(sig.id)} × tidak bisa dibuka (tidak ada rute${exitId ? ` ke ${codeOf(exitId)}` : ""})`,
			);
			return; // stay red
		}
		...
		if (requiredSwitches.length > 0 && ROUTE_SETTING === "manual") {
			...
			const wanted = (label: string): string => {
				...
				const swId = control
					? requiredSwitches
							.map(Number)
							.find((id) => control.ids.includes(id))
		...
		if (requiredSwitches.length > 0) {
			...
			const moveDesc = Object.entries(moves)
				.map(
					([k, v]) => `P${k} ${v === "reversed" ? "BELOK" : "LURUS"}`,
				)
				.join(", ");
		...
		for (const flank of flanks) {
			...
			if (isLocked(flank)) {
				setConflictNote(
					`${codeOf(sig.id)} tidak bisa dibuka — wesel P${flank} terkunci di posisi berbahaya.`,
				);
				window.setTimeout(() => setConflictNote(null), 3000);
				logClick(
					`${codeOf(sig.id)} × tidak bisa dibuka (flank P${flank} terkunci)`,
				);
				return; // stay red
			}
		...
		const routeOccupied = trainStatesRef.current.some((m) => {
			...
			const approaching =
				m.y === sig.lineY &&
				m.dir === sig.dir &&
				(sig.dir === "right" ? m.x < sig.x : m.x > sig.x);
			...
			const loop = Object.values(LOOPS_BY_GROUP_ID).find(
				(l) =>
					Math.abs(l.lineY - m.y) < 1 &&
					m.x >= l.minX &&
					m.x <= l.maxX,
			);
			if (loop) {
				const rejoinX =
					m.dir === "right" ? loop.rejoin.rightX : loop.rejoin.leftX;
				const rejoinY = loop.rejoin.mainLineY;
				if (
					Math.abs(m.x - rejoinX) < 2 * CELL &&
					distToPoly(rejoinX, rejoinY, prospective.pts) < CELL
				) {
					return true;
				}
				return false;
		...
		if (routeOccupied) {
			setConflictNote(
				`${codeOf(sig.id)} tidak bisa dibuka — ada kereta di rute.`,
			);
			window.setTimeout(() => setConflictNote(null), 3000);
			logClick(
				`${codeOf(sig.id)} × tidak bisa dibuka (ada kereta di rute)`,
		...
		const clash = SIGNALS.find((other) => {
			if (other.id === id || other.block || other.dir === sig.dir)
				return false;
			...
			return otherRoute
				? routesOverlap(prospective.pts, otherRoute)
				: false;
		});
		if (clash) {
			setConflictNote(
				`${codeOf(sig.id)} tidak bisa dibuka — rutenya berimpit dengan reservasi ${codeOf(clash.id)}.`,
			);
			window.setTimeout(() => setConflictNote(null), 3000);
		...
		const nextAsp = prospective.nextSignalId
			? aspectOf(prospective.nextSignalId)
			: "green";
	...
	aspectOfRef.current = (id: string, selfIdx?: number) =>
		aspectOf(id, undefined, selfIdx); // keep the tick loop's aspect lookup current
	...
	const stopName = (
		train: {
			...
			stops: { trackmark: string }[];
		},
		idx: number,
	) => {
		const s = train.stops[idx];
		if (
			(s.trackmark === "JNG-W" || s.trackmark === "JNG-E") &&
			idx === 0 &&
			train.neighborBefore
		)
	...
	const trainInfo = (ti: number) => {
		...
		const dirLabel =
			plan.start.dir === "right"
				? `ke timur · ${dirOrigin} → ${dirDest}`
				: `ke barat · ${dirOrigin} → ${dirDest}`;
		...
		const dwellStation =
			st.leg > 0 ? (train.stops[st.leg - 1]?.trackmark ?? null) : null;
		const meetsRel = dwellStation ? meetsRelease(ti, dwellStation) : 0;
		const schedDepart = leg.departAt ?? 0;
		const effectiveDepart =
			leg.departAt !== undefined || meetsRel > 0
				? Math.max(schedDepart, meetsRel)
				: undefined;
		const dwelling =
			st.leg > 0 &&
			effectiveDepart !== undefined &&
			st.time < effectiveDepart;
		...
		if (st.done) {
			...
			if (st.stopReason === "signal")
				status = `Ditahan sinyal ${codeOf(st.stopSignalId ?? "")}`;
			else if (st.stopReason === "junction") status = "Menunggu wesel";
			else if (st.stopReason === "conflict")
				status = "Konflik — kereta bertabrakan";
			else if (st.stopReason === "queue") {
				// Show the train's current location: between which two stations it is.
				const nextStation = stopName(
					train,
					Math.min(st.leg, train.stops.length - 1),
				);
				const prevStation =
					st.leg > 0
						? stopName(train, st.leg - 1)
						: stopName(train, 0);
			...
			const reachedPlatform =
				lastReached >= 0 &&
				train.stops[lastReached].arr !== train.stops[lastReached].dep;
			if (
				reachedPlatform &&
				st.leg > 0 &&
				leg.departAt !== undefined &&
			...
			const heldPastBook =
				meetsRel > schedDepart && st.time > schedDepart;
			if (heldPastBook) {
				// still held by a meet after the book departure — visibly late
				const partners = (
					MEETS_BY_TRAIN.get(ti)?.get(dwellStation!) ?? []
				)
					.map((d) => JOURNEYS[d.partnerIdx].train.train_no)
					.join(", ");
				...
				const meetDeps = dwellStation
					? (MEETS_BY_TRAIN.get(ti)?.get(dwellStation) ?? null)
					: null;
				const partners = meetDeps
					? meetDeps
							.map((d) => JOURNEYS[d.partnerIdx].train.train_no)
							.join(", ")
					: "";
		...
		JOURNEYS.map((j, ti) => {
			...
			const actualSec =
				st?.actualArr[idx] != null
					? (RELATIVE_CLOCK ? j.plan.boundaryArr : 0) +
						st.actualArr[idx]!
					: null;
			return {
				no: j.train.train_no,
	...
	const cellX0 = (sw: Sw, cellSize: number) =>
		Math.floor((sw.x - GRID_OFFSET[0]) / cellSize) * cellSize +
		GRID_OFFSET[0];
	const cellY0 = (sw: Sw, cellSize: number) =>
		Math.floor((sw.y - GRID_OFFSET[1]) / cellSize) * cellSize +
		GRID_OFFSET[1];
	...
	const commonHalf = (sw: Sw, cellSize: number) => {
		...
		if (!match) return "";
		const towardNode =
			Number(match[1]) === sw.x && Number(match[2]) === sw.y;
				...
				<div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 sm:p-7 shadow-2xl">
					...
					<div className="text-center mb-6">
						...
						<p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
							Pilih stasiun dan jam mulai untuk menjalankan
							simulasi.
						</p>
					</div>
					...
					<div className="mb-5">
						<label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
							Pilih Wilayah / Stasiun
						</label>
						<div className="grid grid-cols-1 gap-2.5">
							{STATION_OPTIONS.map((stn) => {
								...
								return (
									<button
										...
										className={`flex flex-col text-left p-3.5 rounded-xl border transition-all cursor-pointer ${
											isSelected
												? "border-green-600 dark:border-green-500 bg-green-50 dark:bg-green-950/40 ring-1 ring-green-600/40 dark:ring-green-500/50"
												: "border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900"
										}`}
									>
										...
										<span className="text-xs text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
											{stn.description}
										</span>
									</button>
								);
					...
					<div className="mb-6">
						...
						<input
							...
							className="w-full rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 px-3.5 py-2 text-sm text-slate-900 dark:text-white font-mono focus:border-green-600 dark:focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-600 dark:focus:ring-green-500"
						/>
							...
							{["00:00", "06:00", "12:00", "18:00"].map((t) => (
								<button
									...
									className={`rounded-lg py-1.5 text-xs font-medium transition-colors cursor-pointer border ${
										startTimeInput === t
											? "border-green-600/60 dark:border-green-500/60 bg-green-100 dark:bg-green-600/20 text-green-800 dark:text-green-400 font-semibold"
											: "border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700 hover:text-slate-900 dark:hover:text-slate-200"
									}`}
								>
									{t}
								</button>
					...
					<button
						...
						onClick={() => {
							...
							initializeSim(sec, chosen);
						}}
						className="w-full rounded-xl bg-green-600 py-2.5 text-sm font-semibold text-white shadow-lg shadow-green-900/20 transition-all hover:bg-green-700 cursor-pointer active:scale-[0.99]"
					>
						Mulai
					</button>
				</div>
		...
		<div className="w-full max-w-[2000px]">
						...
						>
							<div className="flex items-start justify-between gap-3">
								<div>
									...
									<p className="mt-0.5 text-xs text-slate-400">
										Tampilkan tombol wesel &amp; sinyal di
										bawah tabel
									</p>
								</div>
								<button
							...
							<div className="mt-3 border-t border-slate-100 pt-3">
								...
								<ul className="mt-1.5 space-y-1 text-xs text-slate-500">
									<li className="flex items-center gap-2">
										<span
											className="inline-block h-3 w-8 rounded-sm border border-blue-600 bg-blue-200"
											aria-hidden="true"
										/>
										berjalan
									</li>
									<li className="flex items-center gap-2">
										<span
											className="inline-block h-3 w-8 rounded-sm border border-emerald-600 bg-emerald-200"
											aria-hidden="true"
										/>
									...
									<li className="flex items-center gap-2">
										<span
											className="inline-block h-3 w-8 rounded-sm border border-red-600 bg-red-200"
											aria-hidden="true"
										/>
									...
									<li className="flex items-center gap-2">
										<span
											className="inline-block h-3 w-8 rounded-sm border border-amber-600 bg-amber-200"
											aria-hidden="true"
										/>
									...
									<li className="flex items-center gap-2">
										<span
											className="inline-block h-3 w-8 rounded-sm border border-red-900 bg-red-500"
											aria-hidden="true"
										/>
							...
							<div className="mt-3 border-t border-slate-100 pt-3">
								<button
									...
									className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 cursor-pointer"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="14"
            ...
            — grid mode, or schematic layouts that opt into the chrome */}
				{SHOW_GRID && (
					<g stroke="#e7e7e7" strokeWidth={1}>
						{Array.from(
							{ length: Math.round(RIGHT / GRID_PITCH) + 1 },
							(_, i) => (
								<line
									key={`v${i}`}
									x1={GRID_OFFSET[0] + i * GRID_PITCH}
									y1={GRID_OFFSET[1]}
									x2={GRID_OFFSET[0] + i * GRID_PITCH}
									y2={GRID_BOTTOM_Y}
								/>
							),
						)}
						{Array.from(
							...
							(_, i) => (
								<line
									key={`h${i}`}
									x1={0}
									y1={GRID_OFFSET[1] + i * GRID_PITCH}
									x2={RIGHT}
									y2={GRID_OFFSET[1] + i * GRID_PITCH}
								/>
							),
						)}
					</g>
				...
				{SHOW_GRID && (
					<g>
						...
						{Array.from(
							...
							(_, i) => (
								...
								>
									<line
										x1={GRID_OFFSET[0] + i * GRID_PITCH}
										y1={GRID_OFFSET[1] - GRID_TICKS.size}
										x2={GRID_OFFSET[0] + i * GRID_PITCH}
										y2={GRID_OFFSET[1]}
									/>
									<line
										...
										y1={GRID_BOTTOM_Y}
										x2={GRID_OFFSET[0] + i * GRID_PITCH}
										y2={GRID_BOTTOM_Y + GRID_TICKS.size}
									/>
								</g>
							),
						)}
						...
						>
							{Array.from(
								{ length: Math.round(RIGHT / GRID_PITCH) },
								(_, i) => {
									...
									return (
										<g key={`r${i}`}>
											...
											<text
												x={
													RIGHT +
													GRID_TICKS.rightRowLabelOffsetX
												}
												y={y}
											>
												{i / GRID_LABEL_STEP + 1}
            ...
            and is moved right by SHIFT to leave room for the left extension */}
              ...
              no compiled geometry has to be filtered. */}
					{CLOSED_SPANS.map((span) => {
						...
						return (
							<g
								key={`closed-${span.groupId}-${span.fromX}`}
								aria-hidden="true"
							>
								<line
									x1={span.fromX}
            ...
            E3–F3); schematic mode: authored platform shapes at their coordinates */}
					{STATIONS.map((st) => {
						...
						const shapes = STATION_SHAPES.filter(
							(s) => s.code === st.code,
						);
                  ...
                  involved in a live conflict (counter-rotates to stay upright) */}
					...
					{POINT_CONTROLS.map((ctl) => {
						...
						const fitFont = Math.min(
							baseFont,
							((r / cs) * 2 * 1.45) /
								Math.max(1, labelText.length),
						);
						return (
							<g
								key={
									ctl.onSwitchId !== undefined
										? `${ctl.label}-${ctl.onSwitchId}`
										: ctl.ids.join("-")
								}
						...
						return (
							...
							>
								...
								<circle
									...
									cy={lamps[0]}
									r={5 * cs}
									fill={lit[0] ? colors[0] : "#64748b"}
								/>
								<circle
									cx={0}
									cy={lamps[1]}
									r={5 * cs}
									fill={lit[1] ? colors[1] : "#64748b"}
								/>
								<circle
									cx={0}
									cy={lamps[2]}
									r={5 * cs}
									fill={lit[2] ? colors[2] : "#64748b"}
								/>
								{/* label */}
								<text
									x={13 * cs}
					...
					<div className="flex flex-wrap justify-center gap-2">
						{POINT_CONTROLS.filter(
							...
							(ctl, index, all) =>
								ctl.onSwitchId === undefined ||
								all.findIndex(
									(other) => other.label === ctl.label,
								) === index,
						).map((ctl) => {
							const id = ctl.ids[0];
							const reversed = switches[id] === "reversed";
							const locked = isLocked(id);
							const name = ctl.coupled
								? `P${ctl.ids[0]}+P${ctl.ids[1]}`
								: `P${id}`;
							return (
								<button
									key={ctl.ids.join("-")}
						...
						{SIGNALS.map((sig) => {
							...
							return (
								<button
									...
									type="button"
									onClick={(e) => {
										const shift = (e as React.MouseEvent)
											.shiftKey;
									...
									title={
										routeFromRef.current
											? `Shift-klik: rute dari ${codeOf(routeFromRef.current)} ke ${codeOf(sig.id)}`
											: undefined
									}
						...
						<div ref={rosterScrollRef} className="overflow-auto">
							{(() => {
								...
								JOURNEYS.forEach((j, ti) => {
									...
									const d = Math.abs(
										j.train.stops[0].arr - simRef.current,
									);
									if (d < bestD) {
										bestD = d;
										best = ti;
					...
					(() => {
						...
						return (
							<div className="fixed right-4 top-16 z-50 flex max-h-[70vh] w-96 flex-col rounded-xl border border-slate-200 bg-white shadow-lg">
								<div className="flex items-center justify-between border-b border-slate-100 px-4 py-2.5">
									<p className="text-sm font-semibold text-slate-700">
										Jadwal {stationName(selectedStation)}
									</p>
					...
					>
						<div className="flex items-center justify-between border-b border-slate-100 px-4 py-2">
							...
							<div className="flex items-center gap-1.5">
								...
								<button
									...
									aria-label={
										noticesFolded
											? "Bentangkan notifikasi"
											: "Lipat notifikasi"
									}
									title={
										noticesFolded ? "Bentangkan" : "Lipat"
									}
									className="rounded-full p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 cursor-pointer"
								>
									{noticesFolded ? "⌄" : "⌃"}
						...
						{!noticesFolded && (
							<div className="h-40 overflow-auto">
								{notices.map((n) => {
									const dur = n.resolved
										? (n.resolvedDuration ?? 0)
										: Math.max(
												0,
												Math.round(
													simRef.current - n.since,
												),
											);
									return (
										<div
										...
										>
											...
											<div className="min-w-0 flex-1">
												<p
													className={`text-sm font-semibold ${n.resolved ? "text-slate-500 line-through" : "text-slate-800"}`}
												>
													{n.trainNo}
												</p>
				...
				<div className="fixed bottom-4 left-4 z-50">
					{debugOpen && (
						<div className="mb-2 w-96 max-h-72 overflow-auto rounded-lg border border-slate-200 bg-white p-3 shadow-lg">
							...
							{clickLog.length === 0 ? (
								<p className="text-xs text-slate-400">
									No clicks yet — interact with points &amp;
									signals.
								</p>
							) : (
