...
export const createTrains = (
	...
	schedule
		.filter(
			(train) =>
				!enabledTrainNumbers || enabledTrainNumbers.has(train.train_no),
		)
		.map((train) => ({
			...
			stops: train.stops.map((stop) => ({
				...
				...(stop.entryLine === undefined
					? {}
					: { entryLine: stop.entryLine }),
				...
				...(stop.kld_arr === undefined
					? {}
					: { kld_arr: stop.kld_arr }),
				...(stop.kld_dep === undefined
					? {}
					: { kld_dep: stop.kld_dep }),
				...(stop.bua_arr === undefined
					? {}
					: { bua_arr: stop.bua_arr }),
				...(stop.bua_dep === undefined
					? {}
					: { bua_dep: stop.bua_dep }),
			})),
		}));
...
export const spawnPriority = (
	...
	((train.train_name ?? train.name ?? "") === rules.commuterServiceName
		? rules.commuterPenalty
		: 0) + (parseInt(train.train_no, 10) || 0);
...
export function buildJourney(
	...
	const stopX = (code: string): number =>
		stopXFor ? stopXFor(code) : platformX[code];
	...
	const {
		...
		trainTypeSpeedKmh,
		dwell: {
			holdUntilScheduledDepartureByStation,
			minimumStopSeconds,
			relativeAnchors,
		},
	} = rules;
	...
	const lineNodes = atTrackEdge
		...
		: [];
	const spawnMaxX = lineNodes.length
		? Math.max(...lineNodes.map((n) => n.x))
		: maxX;
	const spawnMinX = lineNodes.length
		? Math.min(...lineNodes.map((n) => n.x))
		: minX;
	...
	for (let i = 0; i < stops.length - 1; i++) {
		...
		const km =
			segmentKm[`${fromCode}-${toCode}`] ??
			segmentKm[`${toCode}-${fromCode}`];
		let speed =
			maxU ??
			Math.max(
				minUnitsPerSecond ?? 0,
	...
	for (let i = 0; i < stops.length; i++) {
		const arr =
			i === 0 ? originArr : anchors[i - 1] + legInfo[i - 1].travel;
		schedArr.push(arr);
		const s = stops[i];
	...
	if (originArr > 0) {
		...
		if (atTrackEdge) {
			...
			spawnX =
				dir === "left"
					? Math.max(rawSpawn, maxX + MARGIN)
					: Math.min(rawSpawn, minX - MARGIN);
		}
		...
		const approachMaxU =
			...
			Math.max(
				minUnitsPerSecond ?? 0,
				Math.abs(spawnX - originX) / originArr,
			);
		const approachCapU =
			typeCapKmh !== undefined
				? typeCapKmh / 3.6 / mpuFor(spawnMidX)
				: Infinity;
		legs.unshift({
			waypointX: originX,
			lineY: spawnY,
			speed: Math.min(approachMaxU, approachCapU),
			station: stops[0].trackmark,
		});
	} else {
	...
	const first = Object.values(nodes)
		.filter(
			(n) =>
				n.y === spawnY &&
				(dir === "left" ? n.x < spawnX : n.x > spawnX),
		)
		.sort((a, b) => (dir === "left" ? b.x - a.x : a.x - b.x))[0];
	const firstNode = first
		? (Object.keys(nodes).find((k) => nodes[k] === first) ?? null)
		: null;
	...
	return {
		originArr: 0,
		boundaryArr: originArr,
		approach: originArr > 0,
		start: { x: spawnX, y: spawnY, dir, firstNode, entryX },
		legs,
		schedArr,
	};
}
...
const resolveNode = (
	...
	if (node.sw !== undefined) {
		...
		const branchAhead =
			...
			bearingDot(incomingBearing, branchExit.bearing) > 0;
		const cameFromBranch =
			incoming !== null &&
			branchExit !== undefined &&
			incoming === branchExit.neighbor;
		if (cameFromBranch) {
	...
	const reversed =
		node.sw !== undefined && ctx.switches[node.sw] === "reversed";
	...
	if (
		node.sw !== undefined &&
		!node.exits.some((exit) => exit.viaSwitchPort === "normal")
	) {
...
const segmentLevelOf = (
	...
	return (
		ctx.segmentLevels[`${fromId}|${toId}`] ??
		ctx.segmentLevels[`${toId}|${fromId}`] ??
		0
	);
};

const segmentSpeedOf = (
	ctx: MoveCtx,
	fromId: string | null,
	toId: string | null,
): number | null => {
	...
	return (
		ctx.segmentSpeeds[`${fromId}|${toId}`] ??
		ctx.segmentSpeeds[`${toId}|${fromId}`] ??
		null
	);
};
...
export function buildSegmentLimits(
	nodes: Record<
		string,
		{ x: number; y: number; exits: { neighbor: string }[] }
	>,
	...
	for (const [id, node] of Object.entries(nodes)) {
		for (const exit of node.exits) {
			...
			if (cfg.east && midX > cfg.east.x) {
				...
				kmh = diagonal
					? cfg.turnoutKmh
					: (cfg.east.straightKmhByY[node.y] ?? cfg.turnoutKmh);
			} else {
				mpu = cfg.metresPerUnit;
				kmh = diagonal
					? cfg.turnoutKmh
					: (cfg.straightKmhByY[node.y] ?? cfg.turnoutKmh);
			}
...
export function advanceTrain(
	...
	st.passedSignals = [];
	st.frontPrev =
		st.dir === "left" ? st.x - ctx.trainHalfLen : st.x + ctx.trainHalfLen;
	...
	if (st.stopped) {
		if (st.stopReason === "signal" && st.stopSignalId) {
			if (ctx.aspectOf(st.stopSignalId, st.idx) !== "red") {
				...
				if (now < (st.signalClearedAt ?? Infinity))
					st.signalClearedAt = now;
	...
	while (tRemaining > 0 && !st.done && !st.stopped && guard++ < 200) {
		...
		const departAt =
			leg.departAt !== undefined || meets > 0
				? Math.max(leg.departAt ?? 0, meets)
				: undefined;
		...
		const atStationStart =
			st.leg > 0 &&
			leg.station !== undefined &&
			Math.abs(st.x - stationX) < 1e-6;
		const departureSignal = atStationStart
			? ctx.signals
					...
					.sort((a, b) =>
						st.dir === "right" ? a.x - b.x : b.x - a.x,
					)[0]
			: undefined;
		if (departureSignal) {
			if (
				!ctx.ignoreSignals &&
				ctx.aspectOf(departureSignal.id, st.idx) === "red"
			) {
				st.stopped = true;
				...
				st.x =
					departureSignal.x -
					(st.dir === "left" ? -ctx.trainHalfLen : ctx.trainHalfLen);
				return;
			}
		...
		if (!ctx.ignoreSignals) {
			...
			const segBearing = segHorizontal
				? undefined
				: bearingOf(st.segFrom, st.segTo);
			...
			if (segHorizontal) {
				...
				const tOf = (px: number, py: number) =>
					((px - fx) * dx + (py - fy) * dy) / len2;
				const tFront = tOf(
					st.x + (dx / segLen) * ctx.trainHalfLen,
					st.y + (dy / segLen) * ctx.trainHalfLen,
				);
				ahead = ctx.signals
					.filter(
						(s) =>
							!s.ai &&
							bearingDot(segBearing!, s.bearing) > 0 &&
							tOf(s.x, s.y) > tFront,
					)
		...
		const ahead =
			st.dir === "right" ? leg.waypointX > st.x : leg.waypointX < st.x;
		if (ahead) {
		...
		if (limit.kind === "waypoint") {
			...
			if (
				ctx.stopFrameAtDwellArrival &&
				nextDepartAt !== undefined &&
				st.time < nextDepartAt
			) {
				tRemaining = 0;
			}
	...
	if (toStart > 0 && segLen > 0) {
		if (remaining <= toStart) {
			pieces.push([
				...
				[
					front[0] - ux * remaining,
					front[1] - uy * remaining,
					currentLevel,
				],
			]);
			return pieces;
...
export function occupiedSections(
	...
	if (allHorizontal && footprint.length > 0) {
		...
		const lo = Math.min(
			...footprint.map((p) => Math.min(p[0][0], p[1][0])),
		);
		const hi = Math.max(
			...footprint.map((p) => Math.max(p[0][0], p[1][0])),
		);
		return sections
...
export function reservationAhead(
	...
	const distTo = (p: [number, number]) => {
		...
		for (let i = 0; i + 1 < pts.length; i++) {
			...
			const t = Math.max(
				0,
				Math.min(1, ((p[0] - x1) * dx + (p[1] - y1) * dy) / len2),
			);
			const px = x1 + dx * t;
			const py = y1 + dy * t;
	...
	const pastEndAlongRoute =
		((bestTrain.front - end[0]) * lastDx +
			(bestTrain.lineY - end[1]) * lastDy) /
		lastLen;
	const lateralFromEnd =
		Math.abs(
			(bestTrain.lineY - end[1]) * lastDx -
				(bestTrain.front - end[0]) * lastDy,
		) / lastLen;
	...
	for (let i = 0; i + 1 < pts.length; i++) {
		...
		const t = Math.max(
			0,
			Math.min(1, ((front[0] - x1) * dx + (front[1] - y1) * dy) / len2),
		);
		const px = x1 + dx * t;
		const py = y1 + dy * t;
...
export function signalSections(
	signals: SignalLike[],
): { sig: string; lineY: number; lo: number; hi: number }[] {
	return signals.map((s) => {
		...
		return {
			sig: s.id,
			lineY: s.y,
			lo: Math.min(s.x, farX),
			hi: Math.max(s.x, farX),
		};
	});
}
